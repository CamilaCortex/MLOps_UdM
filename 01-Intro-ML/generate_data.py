...
class UserGenerator:
    ...
    def generate_synthetic_users(self):
        ...
        for i in range(self.n_samples):
            ...
            total_purchases = random.randint(0, 50)  # numero entero de compras, entre 0 y 50
            avg_order_value = random.uniform(10, 500)  # valor promedio de orden, entre 10 y 500
            # 999 es un valor centinela: significa "sin compras todavia" y NO debe tratarse
            # como una cantidad real de dias. Cualquier feature derivada de este campo debe
            # excluir explicitamente los casos con total_purchases == 0 (ver notebook, seccion
    ...
    def add_missing_data(self, df):
        ...
        for column, null_prob in null_config.items():
            if column in df_with_nulls.columns:
                ...
                logger.info(
                    "Agregados %d valores nulos en '%s' (%.1f%%)",
                    null_mask.sum(), column, null_prob * 100,
                )
    ...
    def create_dataset(self):
        ...
        logger.info("Generando datos sintéticos de usuarios para targeting de promociones...")
        ...
        logger.info("Dataset guardado en '%s' (%d filas)", output_file, len(df))
